...
def test_status():
    ...
    assert status(False, "This is False") == {"status": False, "message": "This is False"}


def test_select_query(client, app):
...
def test_value_query(client, app):
    with app.app_context():
        query_result = value_query("insert into items(name) values(%s)", ("TEST_PASS",))
        assert query_result == status(True)
        query_result = value_query("insert into items(name) values(%s)", ("TEST_FAIL", 999))
